...
task Auto_Turn() {
	...
	while(true) {
		...
		if(Turn_Enable) {
			...
			// Set Turn Error
			for(i = 0; i < 5; i++) {
				Turn_PD.Error[i] = Turn_PD.Target;
			} // for(i = 0; i < 5; i++) {
			Angular_Velocity = 0;
			...
			writeDebugStreamLine("Time   Error  Velocity P_Term");
			clearTimer(T1);
			...
			while(Turn_Enable && (Break_Out_Counter < Turn_Break_Out_Counter_Limit)) {
				...
				Angular_Velocity = (1000./(12.*Turn_Refresh_Time))*( 25*Turn_PD.Error[k]
				...
				*/
				...
				writeDebugStreamLine("%4i,  %4i,  %6.3f,   %3i",time1(T1), Turn_PD.Error[k], Angular_Velocity,(signed char)(-P_Turn*Turn_PD.Error[k]));
