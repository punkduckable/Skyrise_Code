...
void Auto_Set_Drive(Byte L_Drive_Power, Byte R_Drive_Power);
	Byte L_Drive_Sign, R_Drive_Sign;
	int L_Drive_Speed, R_Drive_Speed;
...
////////////////////////////////////////////////////////////////////////////////////
 ...
 void Drive(int Power, int Target_Value) {
	...
	DrivePD.Power = Power;
	DrivePD.Target = Target_Value;
	DrivePD.Target_Sign = (Drive_PD.Target > 0) - (Drive_PD.Target < 0);
	...
	while(true) {
		...
		if(Drive_Assist_Enable) {
			...
			while(Drive_Assist_Enable) {
				// Update Drive position. Turn_PD.Offset contains the last Angle Error.
				...
				L_Drive_Correction = P_Drive_Assist*Error[k] + D_Drive_Assist*Angular_Velocity; // Assign L Correction
				L_Drive_Correction = Drive_PD.Target_Sign*L_Drive_Correction; // So that the error is correct if the robot drives backwards
